...
A terminal is a text-based way to talk to your computer. Every click you do in File Explorer — opening a folder, copying a file, deleting something — you can also type as a command.
...
1. Press `Win+S` to open Search (or click the search bar in the taskbar)
...
### `start` — Open in File Explorer or default app
...
> **Try it yourself:** Type `start .` to see your current folder in File Explorer. Recognize those files? They're the same ones `ls` showed you.
