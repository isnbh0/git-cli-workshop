...
<!-- WINDOWS -->
## 2b. SSH 에이전트 시작 (Windows)

SSH 에이전트가 실행 중이어야 키가 작동합니다. 관리자 권한으로 한 번만 설정하면 됩니다.

**1단계: PowerShell을 관리자 권한으로 열기:**

1. `Win+S`를 누르고 "PowerShell"을 입력
2. **PowerShell**을 우클릭 → **관리자 권한으로 실행** 선택

**2단계: SSH 에이전트 활성화 및 시작** (관리자 PowerShell에서):
...
**3단계: 키 추가** (일반 Windows Terminal로 돌아와서):
...
> **참고:** 관리자 권한 단계는 한 번만 하면 됩니다. 이후에는 SSH 에이전트가 자동으로 시작됩니다.
<!-- /WINDOWS -->
...
2. **New SSH key** 클릭
3. **Title:** "{{my_computer_label}}" 같은 이름을 입력 (본인을 위한 라벨입니다)
...
사전 준비에서 이미 `gh`를 설치했습니다 (`winget install --id GitHub.cli`). 설치되어 있는지 확인하세요:
