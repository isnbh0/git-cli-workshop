...
터미널은 텍스트로 컴퓨터에게 말을 거는 방법입니다. {{file_explorer}}에서 클릭으로 하는 모든 것 — 폴더 열기, 파일 복사, 삭제 — 을 명령어로 입력할 수 있습니다.

왜 굳이 이렇게 할까요? 익숙해지면 클릭보다 빠르고 강력하기 때문입니다. 그리고 git을 포함한 많은 개발 도구들은 터미널에서만 작동합니다.

사전 준비에서 설치한 터미널 앱을 사용합니다.

<!-- MACOS -->
> **참고 (macOS):** [Terminal User Guide for Mac — Apple Support](https://support.apple.com/guide/terminal/welcome/mac)
<!-- /MACOS -->
...
<!-- MACOS -->
1. `{{search_shortcut}}`를 눌러 {{search_name}} 열기
2. "iTerm" 입력
3. Enter 누르기
...
<!-- /MACOS -->
<!-- WINDOWS -->
1. `{{search_shortcut}}`를 눌러 {{search_name}} 열기 (또는 작업 표시줄의 검색창 클릭)
2. "{{terminal_app}}" 입력
3. Enter 누르기

PowerShell 프롬프트가 있는 창이 나타납니다. 이것이 여러분의 터미널입니다.
<!-- /WINDOWS -->
...
<!-- WINDOWS -->
Windows Terminal을 열면 다음과 같은 화면이 보입니다:
...
| 부분 | 의미 |
|------|------|
| `PS` | PowerShell (사용 중인 셸) |
| `C:\Users\yourname` | 현재 위치 (홈 폴더) |
| `>` | "입력 대기 중" |
<!-- /WINDOWS -->
...
**하는 일:** 파일 시스템에서 현재 위치 — 현재 폴더의 전체 경로를 보여줍니다.
...
홈 폴더에 있다는 뜻입니다. 현재 위치가 헷갈릴 때마다 `pwd`를 사용하면 됩니다.

> **직접 해보세요:** `pwd`를 입력하고 Enter를 누르세요. 그곳이 홈 폴더 — 새 터미널을 열 때의 시작 위치입니다.
...
### `{{open_cmd}}` — {{file_explorer}} 또는 기본 앱으로 열기

**하는 일:** 더블클릭하는 것처럼 파일이나 폴더를 엽니다.
...
**예상 결과:** 현재 폴더의 내용을 보여주는 파일 탐색기 창이 열립니다.
<!-- /WINDOWS -->

> **직접 해보세요:** `{{open_cmd}} .`을 입력해서 현재 폴더를 {{file_explorer}}에서 확인하세요. 그 파일들이 보이나요? `ls`로 보았던 것과 같은 파일들입니다.
