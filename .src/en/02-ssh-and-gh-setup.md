...
<!-- WINDOWS -->
## 2b. Start the SSH Agent (Windows)

The SSH agent must be running for your key to work. This requires a one-time setup with administrator privileges.
...
**Step 2: Enable and start the SSH agent** (in the administrator PowerShell):
...
**Step 3: Add your key** (back in regular Windows Terminal):
...
> **Note:** You only need to do the administrator step once. After that, the SSH agent starts automatically.
<!-- /WINDOWS -->
...
Select and copy the output (triple-click the line to select it, then copy with `{{Cmd}}+C`).
...
2. Click **New SSH key**
3. **Title:** Give it a name like "{{my_computer_label}}" (this is just a label for you)
...
You already installed `gh` during prerequisites (`winget install --id GitHub.cli`). Verify it's available:
